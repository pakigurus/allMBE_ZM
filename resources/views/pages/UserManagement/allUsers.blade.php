        ...
        <div class="block-content block-content-full">
            ...
            <table class="table table-bordered table-striped table-vcenter js-dataTable-full-pagination">
                ...
                <tr>
                    ...
                    <th class=" d-sm-table-cell" >Is Masajid User?</th>
                ...
                @foreach($users as $key=>$user)
                    <tr>
                        ...
                        <td class=" d-sm-table-cell">
                            <form action="{{url('admin/is-masajid-user/'.$user->id)}}" method="POST">
                                @csrf
                                <div class="custom-control text-center custom-switch custom-control-success mb-1">
                                    <input type="checkbox" class="custom-control-input" id="{{$key+1}}" name="status" @if($user->is_masajid == 1) checked @endif onchange="mySubmit(this.form)">
                                    <label class="custom-control-label" for="{{$key+1}}"></label>
                                </div>
                            </form>
                        </td>
                        <td><i class="fas fa-dot-circle" style="color: {{$user->status == 1 ? "green" : "red"}}"></i></td>
